...
unsigned int crc16Char(unsigned int polinom16, unsigned int init_val_16,unsigned char* adr_start,unsigned int len, int ord)
{
  //rezultatul final
...
//aceasta implementare utilizeazã o tabelã cu valori pre-calculate ale //funcþiei CRC16
...
{
  unsigned long x=*((unsigned long *)(p3));
  int index=0;
  USART_transmit('0');
  USART_transmit('x');
...
void main(void)
{
  ...
  while(1)
  {
    if(flag==1)
    {
      mycrc16=crc16Char(CRC16_CCITT,0,vect,index,MSBF);
      //mycrc16_t=crc16wtable(0,vect,index);
      unsigned int *val1=&mycrc16;
      unsigned int *val2=&mycrc16_t;
      nrCifre=lungimeNumar(mycrc16);
      ...
      for(int i =0;i<index;++i)
      {
       ...
       USART_transmit('C');
       USART_transmit('-');
       hexadecimalTransmit(0,nrCifre,val1);
       USART_transmit('>');
       flag=0;
       index=0;
